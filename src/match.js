...
exports.csrValidation = function (certPem, csrPem, cb) {

  async.parallel([
    openssl.thunk(['x509', '-noout', '-modulus'], certPem),
    openssl.thunk(['req', '-noout', '-modulus'], csrPem)
  ], function (err, results) {
    /* istanbul ignore if */
    if (err) return cb(err);
    cb(null, results[0] == results[1]);
...
exports.csrFiles = function (certFile, csrFile, cb) {

  async.parallel([
    openssl.thunk(['x509', '-noout', '-modulus', '-in', certFile]),
    openssl.thunk(['rsa', '-noout', '-modulus', '-in', csrFile]),
  ], function (err, results) {
    /* istanbul ignore if */
    if (err) return cb(err);
    cb(null, results[0] == results[1]);
